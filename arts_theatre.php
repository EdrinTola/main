...
$stmt = $conn->query("SELECT * FROM events WHERE 
    title LIKE '%Phantom%' OR 
    title LIKE '%Opera%' OR 
    title LIKE '%Swan Lake%' OR 
    title LIKE '%Ballet%' OR 
    title LIKE '%Lion King%' OR 
    title LIKE '%Hamilton%' OR 
    title LIKE '%Musical%' OR 
    title LIKE '%Theatre%' OR 
    title LIKE '%Disney%' OR 
    title LIKE '%Ice%' OR
    title LIKE '%Ballet%'
    ORDER BY event_date");
...
<head>
    ...
    <title>Arts & Theatre | TicketGeek</title>
...
<body>
    ...
    <div class="content-wrapper">
        <section class="hero" style="background: linear-gradient(rgba(10, 10, 35, 0.6), rgba(10, 10, 35, 0.6)), url('https://images.unsplash.com/photo-1503095396549-807759245b35?w=1200&h=500&fit=crop') center/cover;">
            <h2>Arts & Theatre</h2>
            <p>Experience world-class performances and shows</p>
        </section>

        <section class="main-layout" style="grid-template-columns: 1fr;">
        <div class="left-genres">
            <div class="event-row">
                <h2 class="section-title" style="text-align: center;">Upcoming Arts & Theatre</h2>
                <?php if (empty($events)): ?>
                    <p>No arts & theatre events available at the moment.</p>
                <?php else: ?>
